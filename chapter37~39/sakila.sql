...
-- 각 영화별 평균 대여 기간 조회
-- SELECT f.title, AVG(DATEDIFF(r.return_date, r.rental_date)) as avg_rental_preiod
-- FROM film f
...
-- 각 배우별 출연한 영화 수 조회
-- SELECT a.first_name, a.last_name, COUNT(fa.film_id) as numver_of_films
-- FROM actor a
-- JOIN film_actor fa ON a.actor_id = fa.actor_id
...
-- 평균 대여 요금보다 높은 요금의 영호하 조회
-- SELECT f.title, f.rental_rate
-- FROM film f
-- WHERE f.rental_rate > (SELECT AVG(rental_rate) FROM film);

-- 가장 활동적인 고객 조회(가장 많은 영화를 대여한 고객 찾기)
...
-- 영화 대여 상태 변경
UPDATE rental
SET return_date = NOW() -- NOW를 사용하면 현재시간으로 업데이트가 되기 때문에 조심
WHERE rental_id = 200;
